...
class ActCMBPipe:
    ...
    def import_data(self):
        ...
        self.zero_alm = map2alm(zero_map, lmax=self.lmax)
        print("done")
        ...
        self.std_t = np.sqrt(1./self.ivar_t)
        print("done")
        ...
        self.cl_obs = map2cl(self.imap_t, self.lmax)
        self.cl_masked = map2cl(self.imap_t * (1 - self.mask_t), self.lmax)
    ...
    def get_psuedo_cl(self, cl_ref, mask=False):
        ...
        t_tilde_alm = map2alm(t_map, t_tilde_alm)
        cl_psuedo = alm2cl(t_tilde_alm)
        return cl_psuedo
    ...
    def compare_power_spectra(self):
        ls = np.arange(self.lmax + 1)
        plt.figure(dpi=300.)
        # plt.plot(self.cl_obs * ls * (ls + 1) / 2 / np.pi, label='no_mask')
        # plt.plot(self.cl_masked * ls * (ls + 1) / 2 / np.pi, label='masked')
        plt.plot(self.cl_tt_psuedo * ls * (ls + 1) / 2 / np.pi, label='psuedo')
        plt.savefig('plots/ps_compare.png')
        # plt.plot(self.cl_cmb)
    ...
    def compute_a_ksz(self, t_psuedo):
        a_ksz = (self.vr_map * t_psuedo).sum()
        return a_ksz
    ...
    def make_vr_map(self):
        ...
        # self.vr_alm = self.get_zero_alm()
        # ls = np.arange(self.lmax + 1)
        # ms = np.zeros(self.lmax + 1)

        ngal = len(self.gal_pos)
        n_ob = 0
        for i, (v_r, pos) in enumerate(zip(self.v_rad, self.gal_pos)):
            dec, ra = pos
            idec, ira = iround(self.vr_map.sky2pix((dec,ra)))

            if not bounds_check((idec,ira), self.vr_map.shape):
                n_ob += 1
            else:
    ...
    def get_noise_map(self):
        return np.random.normal(size=self.imap_t.shape) * self.std_t

    def compute_sim_spectra(self, make_plots=False):
        self.nl_tilde = map2cl(self.map_fkp * self.get_noise_map(), lmax=self.lmax)
        
        cl_test = np.power(np.ells, -2)
        t_map = alm2map(rand_alm(cl_test))
        cl_tilde = map2cl(t_map * self.map_fkp, lmax=self.lmax)
        fl = cl_tilde / cl_test # verify flatness of fl
    ...
    def compute_estimator(self, ntrial=64):
        ...
        # move import to separate function! (e.g. import...)
        ...
        l_weight = self.beam[:self.lmax + 1,1] * self.cl_ksz / (self.cl_masked * self.cl_zz)
        l_weight[0] = 0.

        t_fkp_est = self.process_t_map(self.imap_t, l_weight)
        self.make_vr_map()

        a_ksz = self.compute_a_ksz(t_fkp_est)

        ls = np.arange(self.lmax + 1)
        a_var = 1./((2 * ls + 1) * self.beam[:self.lmax + 1,1] ** 2 * self.cl_ksz**2 / (self.cl_obs * self.cl_zz)).sum()
        # a_sigma = np.sqrt(a_var)

        print('sigma_ksz: {:.2f}'.format(a_ksz))
        ...
        #   a_samples[i] = self.compute_a_ksz(t_psuedo)
...
if __name__ == "__main__":
    ...
    act_pipe.compare_sim_spectra()
    # act_pipe.compute_estimator(ntrial=4)
