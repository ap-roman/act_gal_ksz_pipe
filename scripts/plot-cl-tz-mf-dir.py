from fnl_pipe.util import OutputManager, get_yaml_dict, average_fl, downgrade_fl, get_fname, parse_act_beam, get_unique_files_prefixes
...
class FitWrapper:
    ...
    def _exp_model_2d(self, alist, llist):
        return alist[:, None, None] * np.exp(-(self.ells_fit[None, None, :] / llist[None, :, None] / 2)**2)

    def _exp_model_1d(self, alist, l):
        return alist[:, None] * np.exp(-(self.ells_fit[None, :] / l / 2)**2)

    def exp_model(self, a, l):
        return a * np.exp(-(self.ells_fit / l / 2)**2)
...
class CltzSFWrapper:
    ...
    @classmethod
    def make(cls, om, freq, cl_tz_sig_path, cl_tz_mc_path, beam_path, weight_path,
             nave_tz, nave_tt=None, label=''):
        if nave_tt is None: nave_tt = nave_tz


        wfkp_tot = enmap.read_map(weight_path)

        fsky = (wfkp_tot * wfkp_tot.pixsizemap()).sum() / 4 / np.pi # fsky normalization factor

        om.printlog(f'{label} fsky: {fsky:.3e}')

        cl_tz_sig = np.load(cl_tz_sig_path) / fsky
        cl_tz_mc = np.load(cl_tz_mc_path) / fsky
        lmax = len(cl_tz_sig) - 1

        bl = parse_act_beam(beam_path)[1][:lmax + 1]

        return cls(om, freq, cl_tz_sig, cl_tz_mc, bl, nave_tz, nave_tt, cl_tz_sig_path, label=label)

    @classmethod
    def make_from_dir(cls, om, freq, paths, nave_tz, nave_tt=None):
        base_path = paths['base_path']
        beam_path = paths['beam']

        dat_paths = get_unique_files_prefixes(base_path, prefixes=['w_tot', 'cl_tz_sig', 'cl_tz_mc'])

        label = base_path.split(os.sep)[-2]
        print(f"Making cltz with label: {label}")

        return cls.make(om, freq, dat_paths['cl_tz_sig'], dat_paths['cl_tz_mc'], beam_path, dat_paths['w_tot'], 
                        nave_tz, nave_tt, label=label)
    ...
    def __init__(self, om, freq, cl_tz_sig, cl_tz_mc, bl, nave_tz, nave_tt, desc_str, label=''):
        self.om = om
    ...
    def fit_exp_with_error(self, llims, ntrial=1024):
        ...
        self.sigma_A = fw_sig.x_to_s(np.abs(np.array((0.5 * (-sigma_a[1] + sigma_a[0]), 0))))[0]
        printlog('s_sig: ' + str(self.s_sig))
        printlog(f'sigma_A {self.sigma_A:.3e}')
...
def get_eta_mass(s_sig, sigma_A):
    sigma_A = sigma_A * 1e-6 * u.K
    A = s_sig[0] * 1e-6 * u.K
    yp = 0.2454
    z_mean = 0.57
    chi_mean = 2194.5e6 * u.parsec
    tcmb = 2.726 * u.K
    omega_t = 6.65e-29 * (u.meter**2) 
    mp = 1.67262192e-27 * u.kilogram

    coeff = tcmb * omega_t * (1 - yp/2) * ((1 + z_mean)/chi_mean)**2

    etam = ((A / coeff) * mp).to(u.solMass)

    l0 = s_sig[1]
    rgas = np.sqrt(2) * chi_mean / l0

    return etam, (sigma_A / A) * etam, rgas, z_mean

def report_params(om, cltzs):
    pl = om.printlog

    for cltz in cltzs:
        etam, sigma_m, rgas_com, z_mean = get_eta_mass(cltz.s_sig, cltz.sigma_A)
        a_mean = 1. / (1 + z_mean)

        pl('============================================================')
        pl(f'Reporting fit for {cltz.label}')
        pl(f'\tM_gas: {etam.value * 1e7:.3e} +- {sigma_m * 1e7:.3e} solar masses (eta=1e-7)')
        pl(f'\tR_gas (comoving): {rgas_com.value * 1e-3:.3e} kpc')
        pl(f'\tR_gas (proper): {a_mean * rgas_com.value * 1e-3:.3e} kpc')
        pl('============================================================')


if __name__ == "__main__":
    om = OutputManager(base_path='output', title='plot-cl-tz-mf-diff', logs=['log'], replace=True)
    printlog = om.printlog
    ...
    # def make_from_dir(cls, om, freq, base_path, beam_path, nave_tz, nave_tt=None):

    cltz_f150 = CltzSFWrapper.make_from_dir(om, 150, paths_f150_randsub, NAVE_TZ, NAVE_TT)
    cltz_f150_vsub = CltzSFWrapper.make_from_dir(om, 150, paths_f150_randsub_vsub, NAVE_TZ, NAVE_TT)
    cltz_f090 = CltzSFWrapper.make_from_dir(om, 90, paths_f090_randsub, NAVE_TZ, NAVE_TT)
    cltz_f090_vsub = CltzSFWrapper.make_from_dir(om, 90, paths_f090_randsub_vsub, NAVE_TZ, NAVE_TT)
    cltzs = [cltz_f090, cltz_f090_vsub, cltz_f150, cltz_f150_vsub]
    ...
    make_cltz_comparison(om, [cltz_f150_vsub, cltz_f150], labels=['f150 vsub', 'f150'], 
                         lmin_plot=5, title_suffix='150 vsub vs 150')
    ...
    make_cltz_comparison(om, cltzs, labels=['f090', 'f090 vsub', 'f150', 'f150 vsub'], 
                     lmin_plot=5, title_suffix='combined')


    report_params(om, cltzs)
