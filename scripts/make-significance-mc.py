...
if __name__ == "__main__":
    om = OutputManager(base_path='output', title='make-significance-mc-simple', logs=['log',])
    ...
    alpha_dict = cross_pipe.compute_estimator(ntrial_mc=NTRIAL_MC) # only do bootstrap
    # printlog('done')

    printlog(alpha_dict)
    alpha_bs = alpha_dict['a_ksz_bootstrap_2']
